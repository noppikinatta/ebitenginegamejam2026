...
- `weapon.go` — `WeaponKind`（Cannon/Shotgun/Sniper/Laser）+ `Weapon` と `Stats(p, fireMult)`。**電力は砲塔全体で共通の「発射倍率」`fireMult` として表現され、発射間隔のみに作用**（`間隔 = round(BaseInterval / fireMult)`、`MinInterval` でクランプ）。ダメージは `BaseDamage × LevelMult^Level`、射程・ビームは定数（energy スケールは廃止）。弾の生存は武器ごとにデータ化：`ProjLife = round(ProjMaxDist / ProjSpeed)`、当たり半径 `ProjRadius`（旧ハードコード `Life:120`/`Radius:5` を置換）。`beamTicksLeft` でビーム照射状態を保持
