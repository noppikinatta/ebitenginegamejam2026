...
- [x] **アイテム吸引（磁石）** — ジェム/ニッパー/ハートは `tracking` フラグ式：一度 `MagnetDist` 内に入ると以降は距離無関係で常時自機へホーミング（高速移動で振り切っても諦めない）。**中ボス（`IsBoss && !Final`）撃破で `PickupMagnet` をドロップ**、取得すると `magnetizeAll` が全ジェム＋全 `Pickup` の `tracking` を一括ON。仮アセット `magnet.png`(16×16) は `make magnet-img`（`tools/genmagnetimg`）
...
- **音声はダミー（差し替え前提）だが配線済み・実際に鳴る**。**SEとBGMで扱いを分離**：
  - **SE**は**`asset/sound/se/` ディレクトリに loose な wav としてコミットし `//go:embed sound/se/*.wav` で直接埋め込み**（BGMと同様。pak・難読化は廃止＝仮素材は自作の波形生成なので隠す必要がない）。**発射音は武器ごとに別ファイル**（`fire_cannon`/`fire_shotgun`/`fire_sniper`/`fire_laser`/`fire_gatling`/`fire_grenade`/`fire_ciws`/`fire_missile`）＋`explosion`/`hit` の計10種＝`sound/se/<name>.wav`。core は `SndFire*` を**武器種ごとに emit**（`core.FireSound(WeaponKind)`、`world.go` の発射時）、scene の `soundSink` が各 `asset.SEFire*` へマップ。同tick・同武器種の多重発射は `DispatchSounds` で1回に間引くが、**別武器種は各々鳴る**。**SEを1個だけ差し替える時は当該 wav を同名で `asset/sound/se/` に置くだけ**（再パック不要）
  - **BGMは自作前提なので隠す必要がなく、`asset/sound/bgm/` ディレクトリごと `//go:embed`**（pak非経由・難読化なし）。**2曲構成**：`bgm_title.wav`（オープニング＋タイトル）と `bgm_game.wav`（ゲーム中＋リザルト）。本物が出来たら各wavを差し替えるだけ。**WASM配信を軽くするためOGG化に対応**：`make bgm-ogg`（=`tools/wav2ogg`、要ffmpeg）が各wavを Ogg Vorbis に変換して `bgm/` に出力し、**WAV時代の正確なループ長**を `<name>.ogg.loop` に記録する（ロッシーでもシームレスループを保つため、ランタイムはこの長さを `NewInfiniteLoop` に渡す。末尾1秒の無音パッドでエンコーダのパディングがループ領域を侵さない）。ループ長計算は Ebiten の `wav.DecodeWithSampleRate`＋`convert.Resampling.Length()` をバイト一致で再現（44100→44100のリサンプルも込み）。**ロードは `decodeBGM` が `<name>.ogg`＋`.ogg.loop` を優先し、無ければ `<name>.wav`（`Length()`）にフォールバック**。ディレクトリごと埋め込むので、oggを確認後に `bgm/*.wav` を消すだけでogg-onlyを配信でき、コード変更不要（**処理の全体像は `docs/bgm-ogg.md`**）。`asset.PlayBGM(BGMTitle|BGMGame)` が**現在再生中と同じトラックなら何もしない**ので、同曲を共有するシーン遷移（オープニング↔タイトル、ゲーム↔リザルト）はシームレス継続、曲が変わる切替時のみ頭から再生。各シーンの `OnStart` で自分のトラックを要求（Opening/Title→`BGMTitle`、InGame/Result→`BGMGame`）
  - 仮音源は波形生成：`make sound-gen` が `gensound` を実行（SE10種→`asset/sound/se/`、BGM2曲→`asset/sound/bgm/`）。`tools/gensound` はサイン/**矩形(`wSquare`)/ノコギリ(`wSaw`)/三角(`wTri`)** の各波形を `osc`/`oscSweep`（波形関数を受ける汎用オシレータ）＋ノイズで合成。既存wavはスキップ（`FORCE=1` で上書き）
  - `asset.LoadSounds()`（`app/main.go` 起動時）が `bgm/` の各トラックを `decodeBGM`（ogg優先・wavフォールバック）でデコード＋`seFS.ReadFile("sound/se/<name>.wav")` でSEをデコード。デコード/欠落は握りつぶす（ログして継続、SE欠落でも無音で起動、BGM失敗もBGM無しで継続）。SEは `context.NewPlayerFromBytes` で毎回生成し多重再生可、BGMはトラックごとに単一プレイヤーを `Rewind` で使い回しループ
- **タイトル背景**：`drawTitle` は**インゲームと同じスクロール背景＋組み上がった自機**を描き、その上に**タイトルロゴ `title.png`（透過・文字のみ・1280×360）を最前面に (0,0) 等倍で重ねる**（旧プレースホルダが真っ黒で全面を覆っていたフルスクリーン敷きは廃止し、背景はインゲームへシームレスに繋がるマップ描画に置換。ロゴは透過画像なので背景の自機・マップが透けて見える）
...
- `weapon.go` — `WeaponKind`（Cannon/Shotgun/Sniper/Laser/Gatling/Grenade/CIWS/Missile）+ `Weapon` と `Stats(p)`。**発射はアキュムレータ方式**：各 `Weapon.fireProgress` が毎tick `fireIncrement(p, fireMult)`（＝発射倍率、`BaseInterval/MinInterval` で上限クランプ）ぶん進み、`BaseInterval` に達したら発射。**新規装着時は `World.primeNewWeapons` が `fireProgress` を `0〜70%×BaseInterval` のランダム初期値でシード**（武器がバラバラに撃ち始める）。`primed` フラグで一度だけ・切断/追加で再ランダム化しない。`ActiveWeapons` はマップ順＝非決定的なので、priming は `TileIdx` ソート順に rng を引いて同一シードの再現性を保つ。平均間隔 = `BaseInterval/fireMult`（`MinInterval` 下限）。**照準と発射は独立**：`AimMode`（`AimLockOn`＝圏内最近接へ/CIWS・Missile／`AimForward`＝常に前方/Gatling／`AimOutward`＝タイルから放射状/Grenade）。**CIWS以外は敵不在でも発射**（`HoldWhenNoTarget` の武器＝CIWSだけ圏内に敵が入るまで満タン保持）。1ショットの弾数は `Pellets`＋`SpreadRad`（`SpreadRandom`＝乱数拡散/Gatling・CIWS、固定均等/Shotgun）、`BurstGap>0` で時間差連射（`pelletsLeft`/`pelletTimer`：Gatling10発3tick間隔、CIWS10発2tick間隔ストリーム）。ダメージは `BaseDamage × LevelMult^Level`。**爆発弾** `ExplodeRadius>0` は寿命切れ時に範囲 `ExplodeDamage`（`Projectile.PassThrough`＝接触無視/Grenade、接触ありで未命中時のみ爆発/Missile）。**ホーミング等の弾移動は `ProjectileMover`（`projectile_mover.go`）で差し替え**：`Steer(p, w)` が毎tick速度を操舵、`Projectile.Mover`（`WeaponParams.Mover` 由来、nil=直進）。`NewHomingMover(turn, maxSpeed, straight)`＝**発射後 `straight` tick は直進（ブーストアウト）してから**最近接敵へ旋回力制限付きで操舵（seek）。直進期間は `Projectile.age`（毎tick加算）で判定し、低速ミサイルが砲塔の外まで飛び出してから追尾を始めるので「発射してから追尾」に見える（実データは `straight=15`）。将来の弾系ジャンク（揺れて登る風船等）も別 Mover として追加可能。弾の生存は `ProjLife = round(ProjMaxDist / ProjSpeed)`、当たり半径 `ProjRadius`。`beamTicksLeft`/`beamAngle` でビーム照射状態を保持。**バレル描画の向き**は `Weapon.aimRender`（毎tick `weaponAim` へ `stepAngle` で平滑化、`RenderAngle()` で公開）：戦闘ミニチュアは各バレルが自分の照準方向を向く（`drawTurretTiles` の `aimBarrels`、ポーズ表示は上向き固定）
- `turret.go` — ヘックスグリッド砲塔。電力は**接続消費タイル数→発射倍率の区分線形補間**（`PowerMultiplier(curve, ConsumerTileCount())`、`Config.PowerCurve` の `PowerPoint{Tiles,Mult}` 列を両端クランプで補間）。`ComputePower`/`WeaponPower` は各タイルの**接続判定**として存続（値は描画の減光のみで参照）。`Component` インターフェースは `Name()` ＋ `Mods() Modifier`（戦車/砲塔への加算修飾子；Wire/WeaponComponent/Junk はゼロ、`Capacitor` は `DamageBonus`）。接続中タイルの `Mods()` を合算した `Turret.Modifiers()` をタイル追加/削除時にキャッシュ再計算する。`Capacitor` は**全武器の攻撃力に1台あたり+10%**（`DamageBonus` を加算合算→ `World.weaponStats` が `1+DamageBonus` を Damage/ExplodeDamage に乗算。永続強化の `DamageMult` とは**乗算**で重なる）。`World.FireRateMultiplier()` はカーブ値そのまま（キャパシタは発射倍率に影響しない）。**自機の移動速度も同じ電力モディファイアに連動**：`World.PlayerSpeed()`＝`Player.Speed`（係数；`defaultPlayer`=1.5、`MetaSpeed`強化で加算）×`FireRateMultiplier()`。タイルが増える＝電力が薄まると武器だけでなく戦車も鈍足になり、切断で再集中させると速くなる（`updatePlayer` と HUD の Spd 表示が共用）。`Modifier` は将来 MaxHP 等の設備へ拡張可能。`distancesFrom` は接続判定・カスケード用。**ジェネレータ（中央タイル）は切断不可の接続ルートだが、武器/ジャンク等のコンポーネントを普通に載せる**（`ConsumerTileCount` からは除外＝薄めないので中央武器は実質「無料の主砲」。`ActiveWeapons` には含まれ発射、`MuzzleOffset` は中央=戦車中心）。`PurgeTile`（+`propagatePurge` カスケード）、`AddTile`（ランダム隣接配置）、`TileCount`/`ConsumerTileCount`、`ActiveWeapons`、`MuzzleOffset`
