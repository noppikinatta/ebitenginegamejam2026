...
- **音声はダミー（差し替え前提）だが配線済み・実際に鳴る**。**SEとBGMで扱いを分離**：
  ...
  - **BGMは自作前提なので隠す必要がなく、`asset/sound/bgm/` ディレクトリごと `//go:embed`**（pak非経由・難読化なし）。**2曲構成**：`bgm_title.wav`（オープニング＋タイトル）と `bgm_game.wav`（ゲーム中＋リザルト）。本物が出来たら各wavを差し替えるだけ。**WASM配信を軽くするためOGG化に対応**：`make bgm-ogg`（=`tools/wav2ogg`、要ffmpeg）が各wavを Ogg Vorbis に変換して `bgm/` に出力し、**WAV時代の正確なループ長**を `<name>.ogg.loop` に記録する（ロッシーでもシームレスループを保つため、ランタイムはこの長さを `NewInfiniteLoop` に渡す。末尾1秒の無音パッドでエンコーダのパディングがループ領域を侵さない）。ループ長計算は Ebiten の `wav.DecodeWithSampleRate`＋`convert.Resampling.Length()` をバイト一致で再現（44100→48000のリサンプルも込み）。**ロードは `decodeBGM` が `<name>.ogg`＋`.ogg.loop` を優先し、無ければ `<name>.wav`（`Length()`）にフォールバック**。ディレクトリごと埋め込むので、oggを確認後に `bgm/*.wav` を消すだけでogg-onlyを配信でき、コード変更不要。`asset.PlayBGM(BGMTitle|BGMGame)` が**現在再生中と同じトラックなら何もしない**ので、同曲を共有するシーン遷移（オープニング↔タイトル、ゲーム↔リザルト）はシームレス継続、曲が変わる切替時のみ頭から再生。各シーンの `OnStart` で自分のトラックを要求（Opening/Title→`BGMTitle`、InGame/Result→`BGMGame`）
