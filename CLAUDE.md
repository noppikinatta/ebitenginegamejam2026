...
- **タイトル画像 `asset/img/title.png`**：本番アセットは 1280×720 想定（`drawTitle` がフルスクリーン等倍描画、自機は手前に重ねる）。差し替え前のプレースホルダは旧 460×300 のまま
...
- タイトルは独立シーンではなく `scene/opening.go` の `drawTitle`（上記）。`title.png` はフルスクリーン（レイアウト等倍）描画なので 1280×720 アセットを置けば 1:1 で出る
