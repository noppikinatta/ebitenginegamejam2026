...
ゲームの audio context は **48000Hz**。実 BGM は **44100Hz / ステレオ / 16bit**。
ランタイムは `wav.DecodeWithSampleRate(48000, …)` で **44100→48000 にリサンプル**
...
`int64(13,891,872 × 48000 / 44100) = 15,120,404` → 4整列済み。
...
記録した `loopLen` は「**44100→48000 のリサンプル**を通した値」。
だから OGG も **44100/ステレオ**のままにして、ランタイムの
`vorbis.DecodeWithSampleRate(48000, …)` が**同じ 44100→48000 リサンプル**を行う
ようにする。こうすると WAV パスと OGG パスのリサンプル経路が一致し、記録した
`loopLen` がそのまま正しいループ点になる。

もし OGG を 48000 で書き出すと、ランタイムはリサンプルせず、`loopLen` の前提
（44100 起点の換算）とズレてしまう。
